...
public final class LocationMethodsAndroid  {
     ...
     */
    @RequiresApi(api = Build.VERSION_CODES.KITKAT)
    public void saveSecretKey(int userId, String secretKey) throws ClassNotFoundException {
        db.insert("contacts", "id,keyy","'"+userId+"','"+secretKey+"'");
    }

    @RequiresApi(api = Build.VERSION_CODES.KITKAT)
    public void saveMyPhoneNumber(String phoneNumber){
        db.insert("myPhoneNumber", "phone_number","'"+phoneNumber+"'");

    }

    @RequiresApi(api = Build.VERSION_CODES.KITKAT)
    public void insertNewNotification(String notification){
        db.insert("newNotifications", "notification","'"+notification+"'");

    }

    @RequiresApi(api = Build.VERSION_CODES.KITKAT)
    public void insertSeenNotification(String notification){
        db.insert("seenNotifications", "notification","'"+notification+"'");
    }

    @RequiresApi(api = Build.VERSION_CODES.KITKAT)
    public void deleteNewNotification(String notification){
        db.delete("newNotifications", "WHERE notification = '"+notification+"'");
    }
     ...
     */
    @RequiresApi(api = Build.VERSION_CODES.KITKAT)
    public synchronized String recvAndDecipher(String msg) throws NoSuchAlgorithmException, NoSuchPaddingException, InvalidKeyException, IllegalBlockSizeException, BadPaddingException,  ClassNotFoundException {
        // recebe a mensagem cifrada e retorna a mesma decifrada
        System.out.println("MENSAGEM CRLH: "+msg);
        JsonObject cipheredMsg = new JsonParser().parse(msg).getAsJsonObject();
        System.out.println("cipheredMsg "+cipheredMsg.get("id").getAsInt());
        SecretKey secretKey = getSecretKey(cipheredMsg.get("id").getAsInt());
        System.out.println("getSecretKey: "+secretKey+"\n"+cipheredMsg.get("id").getAsInt());
        byte[] msg_decrypted = cmiyc.Security.decipher(secretKey, cipheredMsg.get("msg").getAsString());
        System.out.println("message decrypted: "+new String(msg_decrypted));
    ...
    private  SecretKey getSecretKey(int id) throws  ClassNotFoundException {
        String a= db.selectKey("WHERE id='"+id+"'");

       byte[] decodedKey = Base64.decode(a,Base64.NO_WRAP);
       System.out.println("getSecretKey "+a);
    ...
    public Map<String,String> getAllContacts(){
        HashMap<String,String> contacts = db.getAllContacts();
        return contacts;
    }

    @RequiresApi(api = Build.VERSION_CODES.KITKAT)
    public String getMyPhoneNumber(){
        return db.getMyPhoneNumber();
    }

    @RequiresApi(api = Build.VERSION_CODES.KITKAT)
    public ArrayList<String> getNewNotifications(){
        return db.getNewNotifications();
    }

    @RequiresApi(api = Build.VERSION_CODES.KITKAT)
    public ArrayList<String> getSeenNotifications(){
        return db.getSeenNotifications();
    }
    ...
    public void createConcts() throws ClassNotFoundException, InvalidKeySpecException, InvalidAlgorithmParameterException{
        ...
        try {
            ...
            */
            ...
            System.out.println(" gerar secretKey "+Base64.encodeToString(key.getEncoded(),Base64.NO_WRAP));

            saveSecretKey(5, Base64.encodeToString(key.getEncoded(), Base64.NO_WRAP));
            saveSecretKey(6,Base64.encodeToString(key.getEncoded(), Base64.NO_WRAP));
    ...
    private static final class SQLite extends SQLiteOpenHelper {
        ...
        private void createTables(SQLiteDatabase db) {
            ...
            //db.execSQL("DROP TABLE meetingPoints;");
            //db.execSQL("DROP TABLE anchors;");
            ...
            String sql3 = "CREATE TABLE IF NOT EXISTS contacts (\n"
                    ...
                    + " keyy text NOT NULL\n"
                    + ");";
            String sql4 = "CREATE TABLE IF NOT EXISTS groups (\n"
                    + "	id text PRIMARY KEY,\n"
                    + " keyy text NOT NULL,\n"
                    + " group_id text NOT NULL\n"
                    + ");";
            String sql5 = "CREATE TABLE IF NOT EXISTS myPhoneNumber (\n"
                    + "	phone_number text PRIMARY KEY\n"
                    + ");";
            String sql6 = "CREATE TABLE IF NOT EXISTS newNotifications (\n"
                    + "	notification text PRIMARY KEY\n"
                    + ");";
            String sql7 = "CREATE TABLE IF NOT EXISTS seenNotifications (\n"
                    + "	notification text PRIMARY KEY\n"
                    + ");";
        ...
        public void insert(String table, String columns, String row) {
            ...
            try ( // get writable database as we want to write data
                ...
                System.out.println("<"+row+"> ALREADY INSERTED IN <"+table+"> ON COLUMN <"+columns+">");
                e.printStackTrace();
            }
        }
        ...
        @RequiresApi(api = Build.VERSION_CODES.KITKAT)
        public String getMyPhoneNumber(){
            String sql = "SELECT * FROM myPhoneNumber";
            String phoneNumber="";
            Cursor cursor;
            try(SQLiteDatabase db = this.getWritableDatabase()){
                cursor = db.rawQuery(sql, null);
        ...
        @RequiresApi(api = Build.VERSION_CODES.KITKAT)
        public ArrayList<String> getNewNotifications(){
            String sql = "SELECT * FROM newNotifications";
            ArrayList<String> newNotifications = new ArrayList<>();
            Cursor cursor;
            try(SQLiteDatabase db = this.getWritableDatabase()){
                cursor = db.rawQuery(sql, null);
                while(cursor.moveToNext()){
                    newNotifications.add(cursor.getString((cursor.getColumnIndex("notification"))));
                }
            }catch(Exception e){
                e.printStackTrace();
            }
            System.out.println("<<<< NEW NOTIFICATIONS >>>>"+newNotifications);
            return newNotifications;
        }

        @RequiresApi(api = Build.VERSION_CODES.KITKAT)
        public ArrayList<String> getSeenNotifications(){
            String sql = "SELECT * FROM seenNotifications";
            ArrayList<String> newNotifications = new ArrayList<>();
            Cursor cursor;
            try(SQLiteDatabase db = this.getWritableDatabase()){
                cursor = db.rawQuery(sql, null);
            ...
            System.out.println("<<<< SEEN NOTIFICATIONS >>>>"+newNotifications);
            return newNotifications;
        }
        ...
        public HashMap<String,String> getAllContacts(){
            ...
            try(SQLiteDatabase db = this.getWritableDatabase()){
                cursor = db.rawQuery(sql, null);
                System.out.println("lista contactos "+DatabaseUtils.dumpCursorToString(cursor));
                while(cursor.moveToNext()){
                    contacts.put(cursor.getString(cursor.getColumnIndex("id")), cursor.getString(cursor.getColumnIndex("keyy")));
                }
            }catch(Exception e){
                e.printStackTrace();
            }
        ...
        public String selectKey(String whereClause)  {
            ...
            try (SQLiteDatabase db = this.getWritableDatabase()){
                cursor = db.rawQuery(sql, null);
                while(cursor.moveToNext()){
                    key = cursor.getString(cursor.getColumnIndex("keyy"));
                }
            } catch (Exception e) {
                System.out.println(e.getMessage());
            }
